...
# plotly figure
fig1 = px.line(df, x='Time', y='s1', template='plotly_dark', height=400)
fig2 = px.line(df, x='Time', y='s2', template='plotly_dark', height=400)
fig3 = px.line(df, x='Time', y='s3', template='plotly_dark', height=400)
...
def streamFig1(value):
    ...
    fig1 = go.Figure()
    fig1 = px.line(df1, x='Time', y='s1', template='plotly_dark', height=400)
...
def streamFig1(value):
    ...
    anomalies = level_shift_ad.fit_detect(df2)
    fig2 = go.Figure()
    fig2 = px.line(df2, x=df2.index, y='s2', template='plotly_dark', height=400)
    ...
    fig2.add_trace(
        go.Scatter(x=anomalies[anomalies['s2'] == True].index, y=df2[anomalies['s2'] == True].s2.values,
                   mode='markers', fill='toself', fillcolor='rgba(255,144,111,0.5)', name="Anomaly"))
...
def streamFig1(value):
    ...
    fig3 = go.Figure()
    fig3 = px.line(df3, x=df3.index, y='s3', template='plotly_dark', height=400)
    fig3.add_trace(go.Scatter(x=df3.index[-3:], y=df3['s3'][-3:], mode='markers+text', text=df3['s3'][-3:],
                              textposition='top center', fill='none', showlegend=False, texttemplate='%{text:.2f}',
                              name='Sensor 3 values'))
    fig3.add_trace(
